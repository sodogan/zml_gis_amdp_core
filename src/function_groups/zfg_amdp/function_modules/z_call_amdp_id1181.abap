FUNCTION z_call_amdp_id1181
  ...
  TRY.
      ...
      IF NOT go_amdp_logger IS BOUND.
        go_amdp_logger = NEW zcl_amdp_application_logger( subobject = 'ZID1181' ).
      ENDIF.

      DATA(lref_gis_amdp) =  CAST zcl_gis_amdp_id1181( zcl_gis_amdp_base=>get_instance( iv_id = '1181'  ) ).

      ASSERT ID zml_gis_amdp CONDITION  lref_gis_amdp  IS BOUND.

**Call the AMDP method passing the input parameters
      lref_gis_amdp->get_storage_modifica(
        EXPORTING
